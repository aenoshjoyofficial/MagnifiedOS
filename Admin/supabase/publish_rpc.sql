...
BEGIN
    ...
    LOOP
        ...
        FOR v_lesson IN SELECT * FROM jsonb_to_recordset(v_module.lessons) AS y(id UUID, title TEXT, day_number INTEGER, unlock_day INTEGER, description TEXT)
        LOOP
            v_incoming_lesson_ids := array_append(v_incoming_lesson_ids, v_lesson.id);

            INSERT INTO lessons (id, module_id, title, description, day_number, unlock_day)
            VALUES (v_lesson.id, v_module.id, v_lesson.title, v_lesson.description, v_lesson.day_number, v_lesson.unlock_day)
            ON CONFLICT (id) DO UPDATE SET
                title = EXCLUDED.title,
                description = EXCLUDED.description,

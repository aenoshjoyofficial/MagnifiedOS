...
async function run() {
  console.log("================= ALL OLD TASK COMPLETIONS =================");
  const res = await fetch(`${url}/task_completions?select=*`, { headers });
  const completions = await res.json();
  console.log(`Total old completions in database: ${completions.length}`);
}
